...
###   Loading the data   ###
# Basis points
points = np.loadtxt('B737.inp', skiprows=9, max_rows=6597-9, delimiter=',')
points[:, 1:] /= 1000.  # Scale from mm to m

# Rectangular elements
s4r_elements = np.loadtxt('B737.inp', skiprows=6598, dtype=int, max_rows=13232-6598, delimiter=',')

# Assembly elements, where loads are applied
assembly_nodes = np.zeros((16, 4))

for i in range(16):
    assembly_nodes[i, :] = np.loadtxt('B737.inp', skiprows=14146 + 2 * i, max_rows=1, delimiter=',')
